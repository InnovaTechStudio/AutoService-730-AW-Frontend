<template>
  <div class="dashboard-container">
    <h1 class="mb-4">Panel de Control - AutoService</h1>
        ...
        <Card class="kpi-card shadow-1 border-left-blue">
          <template #content>
            <div class="flex justify-content-between">
              <div>
                <span class="block text-500 font-medium mb-3">Vehículos en Taller</span>
      ...
      <div class="col-12 md:col-6 lg:col-3">
        <Card class="kpi-card shadow-1 border-left-orange">
          <template #content>
            <div class="flex justify-content-between">
              <div>
                <span class="block text-500 font-medium mb-3">Órdenes Activas</span>
          ...
          <template #content>
            <div class="flex justify-content-between">
              ...
              <div class="flex align-items-center justify-content-center bg-teal-100 border-round" style="width:2.5rem;height:2.5rem">
                <i class="pi pi-check-square text-teal-500 text-xl"></i>
              </div>
            </div>
          </template>
        </Card>
      ...
      <div class="col-12 md:col-6 lg:col-3">
        <Card class="kpi-card shadow-1 border-left-green">
          <template #content>
            <div class="flex justify-content-between">
              <div>
                <span class="block text-500 font-medium mb-3">Ingresos Proyectados</span>
    ...
    <div class="grid">
      <div class="col-12 lg:col-7">
        <div class="card p-4 bg-white border-round shadow-1 h-full flex flex-column">
          <h3>Estado de la Flota Activa</h3>
          <p class="text-500 text-sm mt-0 mb-4">No incluye vehículos entregados</p>
      ...
      <div class="col-12 lg:col-5">
        ...
        <div class="card p-4 bg-white border-round shadow-1 mb-4">
          <h3 class="m-0 mb-3">Acciones Rápidas</h3>
          <div class="grid">
            <div class="col-12 md:col-4">
              <Button label="Cliente" icon="pi pi-user-plus" severity="secondary" outlined class="w-full" @click="router.push('/customers')" />
            </div>
            <div class="col-12 md:col-4">
              <Button label="Vehículo" icon="pi pi-car" severity="secondary" outlined class="w-full" @click="router.push('/vehicles')" />
            </div>
            <div class="col-12 md:col-4">
              <Button label="Orden" icon="pi pi-file-edit" class="w-full" @click="router.push('/work-orders/new')" />
            </div>
          </div>
        </div>
        ...
        <div class="card p-4 bg-white border-round shadow-1 h-full">
          <div class="flex justify-content-between align-items-center mb-3">
            ...
            <Button label="Ver todas" text size="small" @click="router.push('/work-orders')" />
          </div>
...
const vehiclesInWorkshop = computed(() =>
    vehicleStore.vehicles.filter(v => v.status === 'En Taller').length
);
const activeOrders = computed(() =>
    workOrderStore.workOrders.filter(wo => wo.status === 'En Proceso').length
);
...
const projectedIncome = computed(() =>
    workOrderStore.workOrders
        .filter(wo => wo.status === 'En Proceso')
        .reduce((acc, wo) => acc + (parseFloat(wo.price) || 0), 0)
);
